...
func x_movement(delta: float) -> void:
	...
	var slope_angle: float = get_custom_floor_angle()#Nova função que criei
	
	if abs(slope_angle) > 5.0:
		
	#if slope_angle != 0:
		var slope_factor: float = 1.0 - slope_angle / 90.0
		velocity.x += x_dir * accel_rate * delta * slope_factor
	else:
		# Accelerate
	...
	if x_dir == 0 and is_on_floor():
		var decel_sign: float = sign(velocity.x)
		velocity.x -= deceleration * delta * decel_sign
		
	move_and_slide()

		
func get_custom_floor_angle() -> float:
	if is_on_floor():
...
func apply_gravity(delta: float) -> void:
	...
	# No gravity if we are grounded and not on a slope
	#if is_on_floor() and abs(floor_normal().angle_to(Vector2.UP)) < floor_max_angle:
		#return
